...
方法里的代码保存在属性表的一个名为 `Code` 的属性中，也留到最后和文件的属性表一起分析。
...
### 9.2 方法名称（name_index）
权限修饰符后紧跟的 2 个字节为方法名称索引，这里索引值指向的是该方法在常量池中对应的简单名称。

### 9.3 描述符索引（descriptor_index）
方法名称后面跟的是一组描述符索引，总体分为两个部分，全部塞到 2 字节大小的空间中：
* 开头是参数列表部分，参数描述符严格按参数顺序放在一组 `()` 括号中。
* 结尾是返回值描述符。

例如本文的示例代码中 `calculate` 方法的描述符为
> (I)D

## 10. 属性表
属性表的定义相对宽松，不需要具有严格顺序，不在虚拟机规范中的属性也可以写进class（只是虚拟机运行时会忽略不认识的属性），可以出现在 class 文件中的多个位置，包括方法表、字段表、类的附加属性，甚至属性表中也可以有属性表（比如 Code 属性中包含其他属性表）。
